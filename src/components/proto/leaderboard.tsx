...
import { displayAuthorName, GUEST_AUTHOR_LABEL, USERNAME_MAX, USERNAME_MIN } from "@/lib/zaun/username";
 ...
 *
 * Guests without a chosen name show as "Guest". Set a name *before* saving fences
 * so new verified rows carry that label (older Guest rows stay Guest on the server).
 */
export function Leaderboard({
  ...
  const displayName = displayAuthorName(username || GUEST_AUTHOR_LABEL);
  const listed = verified > 0;

  // Normalize + merge Guest duplicates from the API.
  const rows = (() => {
    const merged = new Map<string, number>();
    for (const r of board ?? []) {
      const name = displayAuthorName(r.name);
      merged.set(name, (merged.get(name) || 0) + (r.verified || 0));
    }
    return [...merged.entries()]
      .map(([name, verifiedCount]) => ({ name, verified: verifiedCount }))
      .sort((a, b) => b.verified - a.verified);
  })();

  const display = listed
    ? (() => {
        const hasMe = rows.some((r) => r.name === displayName);
        const next = hasMe
          ? rows.map((r) =>
              r.name === displayName
                ? { ...r, verified: Math.max(r.verified, verified) }
                : r,
            )
          : [...rows, { name: displayName, verified }];
        return next.sort((a, b) => b.verified - a.verified);
      })()
    : rows;
